...
router.post('/generate', authenticateToken, async (req, res) => {
  try {
    ...
    const characterDirectivesMap = {
      '1': '자기 고백형 서사로 내면의 솔직한 이야기를 말하는 구조',
...
**4.지시문 작성 규칙 (괄호 안 내용):**
- 지시문 최소화: 한 장면에 3~4개만 사용. 감정 변화나 중요한 동작에만 넣기.
 ...
 **4.1 위치**
 대사 앞: 장면 분위기나 인물 상태를 짧게 설명 (예: (잠시 망설이다), (웃음을 참으며), (작게 한숨 쉬며))
 대사 중간: 흐름 중 중요한 감정 전환을 표시 (예: "그 사람… (목소리가 떨리며) 다시 못 볼 줄 알았어.")
 대사 뒤: 대사의 뉘앙스를 보강 (예: "고마워." (진심을 담아))

 **4.2 길이와 형태**
 보통 3~6자 정도의 짧은 동사·형용사 중심
 행동 + 감정 or 감정 단독
 (고개를 돌린다)
 (울먹이며)
 (차갑게)
 ...
 **4.3 사용 빈도**
 한 장면에 너무 많지 않게 → 평균 3~5줄 대사에 한 번 정도
 감정 변화가 크지 않으면 생략
 배우가 스스로 해석할 수 있는 부분은 과도하게 지시하지 않음
