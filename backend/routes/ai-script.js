...
router.post('/generate', authenticateToken, async (req, res) => {
  try {
    ...
    // 길이 변환
    const lengthMap = {
      short: "짧은 대본. 지시문을 제외하고 **배우의 대사 줄 수가 정확히 6~8줄**이어야 한다. 반드시 이 범위를 벗어나지 말 것.",
      medium: "중간 길이 대본. 지시문을 제외하고 **배우의 대사 줄 수가 정확히 12~16줄**이어야 한다. 반드시 이 범위를 벗어나지 말 것.",
      long: "긴 대본. 지시문을 제외하고 **배우의 대사 줄 수가 정확히 20~30줄**이어야 한다. 반드시 이 범위를 벗어나지 말 것."
    };
...
**5. 대본 생성 형식:**
다음 형식을 정확히 따라 작성하세요. === 기호는 사용하지 말고 중복하지 마세요.

제목: [감정이나 상황을 압축한 제목]

상황 설명: [어떤 상황에서 누구에게 하는 말인지, 왜 이런 감정 상태인지 3-4줄로 설명]
...
상황 설명: 지원은 취업 준비 스트레스로 불안에 시달리고 있다. 부모님 기대와 친구들 성공 소식이 압박을 가중시켰다.
...
연기 팁:
- 초반에는 담담하게 시작할 것
- 중반부에서는 불안과 좌절감을 드러낼 것
- 후반부에는 결단력 있는 톤으로 마무리할 것
