...
**3. 대본 작성 지침**
 ...
 - 지시문은 전체 대사의 20~30% 이내로 제한하고, 같은 인물의 연속된 대사에 반복적으로 사용하지 않는다.
...
**5. 대본 대사 줄바꿈 규칙**
 - 화자 변경 시
 - 화자가 바뀌면 반드시 줄바꿈을 하여 새로운 줄에서 대사를 시작하고, 새로운 화자의 이름과 콜론(:)을 명확히 표시한다.
 - 같은 인물의 대사라도 다음 경우 줄바꿈을 한다.
 - 감정 변화가 뚜렷할 때: 대사의 분위기나 감정이 바뀌면 줄을 바꿔서 구분한다.
 - 예) 분노에서 슬픔, 놀람에서 침착함 등
 - 주제가 바뀌거나 대화의 흐름이 전환될 때: 이야기 내용이나 화제가 달라지면 한 줄 띄어 구분한다.
 - 말이 길어질 경우: 한 줄에 2~3문장 이상 넘어가면 적절히 나눠서 읽기 쉽고 리듬감 있게 표현한다.
 - 장면 전환이나 시간이 흐르는 묘사가 있을 때는 대본 내에서 최소 1줄 이상 띄어준다.
 - 필요시 [장면 전환], [시간 경과] 등의 문구를 별도 줄에 표기한다.
 - 감정이 고조되거나 중요한 순간은 짧은 문장 위주로 줄을 나눠 감정의 호흡을 살린다.
 - 반대로 차분하거나 일상적인 대화는 문장들을 묶어 한두 줄로 작성해 리듬감을 조절한다.


**6. 지시문 규칙**
 - 행동, 톤, 감정 등을 적절히 사용한다.
 - 지시문은 대사와 구분해 전·후로 줄바꿈. 
 - 지시문은 문장 중간에 삽입하지 않고, 독립된 줄로 배치.
 - 지시문은 대사의 20~30% 이내로 제한. 예) 대본의 총 대사가 10줄이라면 2~3개로 제한.
 - 같은 인물의 연속된 대사에 반복적으로 사용하지 않는다.
