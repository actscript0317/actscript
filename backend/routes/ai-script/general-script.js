...
2. **관계 맥락**: 상대와의 관계,(예: 친구, 연인, 가족, 동료 등)를 구체적으로 드러내세요.
...
2. **분량 준수**: 지정된 대사 줄 수에 정확히 맞춰 작성하세요.
3. **자연스러운 대사**: 각 인물의 특성에 맞는 자연스러운 대화를 작성하세요.
4. **연기 가능한 지시문**: 배우가 실제로 연기할 수 있는 구체적인 지시문을 포함하세요.

**중요**: 섹션 헤더(===상황 설명===, ===등장인물===, ===대본===, ===연기 팁===)를 정확히 사용하세요. 대본은 반드시 한국어로 작성하며, 표준 대본 형식을 따르세요.`
      },
      {
        role: "user",
