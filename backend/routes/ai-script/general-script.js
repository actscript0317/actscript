...
**🎭 무대 지시문 작성 원칙**
- 무대 지시문은 배우가 실제로 연습할 수 있는 외적 행동·표정·톤만 포함한다. (내적·추상적 지시 금지)
- 모든 대사에 넣지 말고, 평균 3~5줄에 1회 정도만 넣는다. (35줄 기준 7~10개 권장)
- 지시문은 감정 전환·관계 변화를 보여주는 중요한 순간에만 삽입한다.
- 표현은 간결하면서도 구체적으로 작성한다.
  - 예: "너 왜 그래?" → "너 왜 그래? (고개를 돌리며, 목소리를 낮추고)"
  - 예: "그만해!" → "그만해! (주먹을 움켜쥐고, 목소리를 떨며)"
- 초반에는 작은 몸짓·시선, 중반에는 호흡과 긴장, 후반에는 큰 제스처와 강렬한 목소리 변화로 점진적 감정 축적을 보여준다.
...
대본 츌력 예시(내용은 제외하고 대사 및 지시문의 자리와 줄바꿈을 참고할 것.)
솔직히… 이 말 꺼내기가 쉽지 않았어.
우리 그냥 잘 지내고 싶다는 생각도 많았거든.
...
(숨을 고르며, 솔직하게)
나, 너 좋아해.
친구로서가 아니라… 진짜 여자 사람으로서.

네가 웃으면 괜히 같이 기분 좋아지고,
네가 힘들어하면 나도 괜히 신경 쓰이고.
그게 그냥 친구 마음은 아닌 것 같더라.

(조금 쑥스러워하며 웃는다.)
네가 어떻게 생각할진 모르겠는데…
내 마음은 그렇다는 거, 그냥 알아줬으면 좋겠어.
...
1. [민수가 웃는다] ← 지시문에 번호
1. 민수: 안녕하세요
   영희: 반갑습니다 ← 연속 대사에 번호 없음

**중요:** 대사만 번호를 붙이고, 무대 지시문 [괄호 안 내용], 상황 설명 (괄호 안 내용), 빈 줄에는 절대 번호를 붙이지 마세요.
