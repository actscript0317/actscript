...
const sendEmail = async (options) => {
  try {
    ...
    // Sandbox 제한이나 기타 에러 시 시뮬레이션으로 처리
    console.log('🔧 [Sandbox 제한] 이메일 발송 실패, 시뮬레이션 모드로 전환');
    console.log('📧 인증 코드가 콘솔에 출력됩니다.');
    return { message: 'Sandbox 환경 - 이메일 시뮬레이션 완료 (에러 발생으로 인한 fallback)' };
